...
## 2026-07-28 全商品カード画像QA

- 対象: 全11LP、55商品
- 新規取得: 50商品画像。既存のクレンジング画像5点と合わせ、全商品を実物画像で表示
- 画像出典: 各カードの楽天商品ページ `og:image`、ライースのみ公式商品ページ
- 安定化: アイケア5商品とクレンジングのオルビス画像を外部バナー依存からローカル画像へ変更
- 操作: 55/55商品で画像全体が購入先または公式商品ページへのリンク
- ブラウザ確認: 11/11 LP、商品画像55/55点の自然幅取得、破損0、横方向のはみ出し0
- 表示確認: 日焼け止めLPと毛穴美容液LPの商品カードをデスクトップで目視確認
- ビルド: `npm.cmd run build` passed
- 既存テスト: スターター削除済みファイルとCloudflare独自スキームを参照する旧テスト2件のみ失敗（今回変更箇所とは無関係）
